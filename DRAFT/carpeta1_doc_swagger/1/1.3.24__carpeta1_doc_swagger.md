...
<h1 id="cross-recibos">CROSS - RECIBOS v1.3.24</h1>

> 3 Scroll down for code samples, example requests and responses. Select a language for code samples from the tabs above or the mobile navigation menu.

CAMBIO 121!  API que recoge todas las funcionalidades relativas a los recibos gestionados por la Mutua  Madrileña en su distintos.
...
```java
URL obj = new URL("https://$(catalog.host)/cross-recibos/api/v1/polizas/{id-poliza}/recibos?desdeFecha=2020-04-02&hastaFecha=2020-04-02");
HttpURLConnection con = (HttpURLConnection) obj.openConnection();
con.setRequestMethod("GET");
int responseCode = con.getResponseCode();
BufferedReader in = new BufferedReader(
...
$.ajax({
  ...
  method: 'get',
  data: '?desdeFecha=2020-04-02&hastaFecha=2020-04-02',
  headers: headers,
  success: function(data) {
    console.log(JSON.stringify(data));
  }
...
```java
URL obj = new URL("https://$(catalog.host)/cross-recibos/api/v1/recibos-by-nif?fechaConsulta=2020-04-02");
HttpURLConnection con = (HttpURLConnection) obj.openConnection();
con.setRequestMethod("GET");
int responseCode = con.getResponseCode();
BufferedReader in = new BufferedReader(
